...
from GraphGenerator import store_inlinks
...
class Crawler:
    ...
    CRAWLED_FILES = 'Task 1 BFS Crawled Urls.txt'

    def __init__(self):
        self.sleep_time = 1
        self.urls_crawled = {}
        self.pages_set=[]
        self.inlink_graph = defaultdict(set)
    ...
    def crawl(self, base_url, filename=None, output=False, max_depth=5):
        ...
        if base_url in self.urls_crawled:
            return
        bfs_output=self.breadth_first_search(base_url)
        store_inlinks(self.inlink_graph,self.pages_set,"G1 Graph.txt", " ")
        return bfs_output
    ...
    def breadth_first_search(self, base_url):
        ...
        #self.pages_set.append(base_url.split('/').pop())
        # store url and depth
        ...
        while len(queued_urls):
            ...
            base_url_split=base_url.split('/').pop()
            self.pages_set.append(base_url_split)
            ...
            for url in urls:
                if url not in self.urls_crawled:
                    ...
                    queued_urls.append(depth)
                    #self.pages_set.append(url.split('/').pop())
                url_split=url.split('/').pop()
                self.inlink_graph[url_split].add(base_url_split)
    ...
    def get_html_content(self3335TAqY, base_url):
        html_content = None
    ...
    def get_urls_to_crawl(self, base_url, html):
        ...
        for url in html.soup.find_all('a'):
            ...
            # check for unique urls and discard administrative links
    ...
    def _write_to_file(self, base_url, depth):
        # write
        with open(self._filename, 'a') as _file_:
    ...
    def main(self):
        ...
        self.crawl('https://en.wikipedia.org/wiki/Sustainable_energy', 'Task 1 BFS Crawled Urls.txt', output=True)
