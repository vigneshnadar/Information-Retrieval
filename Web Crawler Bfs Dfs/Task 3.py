...
class Crawler:
    ...
    DIR_PATH_FILES = 'Task 3 Downloaded Files'
    CRAWLED_FILES = 'Task 3 BFS Crawled Urls.txt'
    ...
    def get_urls_to_crawl(self, base_url, html):
        ...
        for url in html.soup.find_all('a'):
            ...
            if url and url not in urls_unique and url != base_url and not any(word in url for word in self.exclude_content) and 'https://en.wikipedia.org/wiki/' in check_url.lower() and not url.startswith('#'):
    ...
    def main(self):
        ...
        self.crawl('https://en.wikipedia.org/wiki/Solar_power', 'Task 3 BFS Crawled Urls.txt', output=True)
