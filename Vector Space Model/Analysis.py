...
class CosineSimilarityRank:
    ...
    def __init__(self):
        ...
        self.qidf=0
        # self.docs={'NellisSolarPowerPlant','California','Solarenergy','Solarpower',
        #            'Photovoltaicpowerstations','Solarthermal','NevadaSolarOne',
        #            'SolarEnergyGeneratingSystems'}
        # self.docs = {'Globalwarming','Carbondioxideemissions','Negativecarbondioxideemission','JosephRomm',
        #              'Personalcarbontrading','Climatechangemitigation','Climatechange'}
        # self.docs = {'Renewableenergypayments','RenewableEnergyCertificate','RenewableEnergyCertificates','RenewableenergyinIndia'
        #             ,'Cleanenergy','Sustainableenergy','Renewableenergypolicy','Renewableenergycommercialization','Greeneconomy'}
        self.docs = {'Incandescentlightbulbs','Incandescentlightbulb','Energysavinglamp','LEDlamp',
                     'Phaseoutofincandescentlightbulbs','LEDlights'}
    ...
    def RunTask(self,query_id,query):
        ...
        # print self.inverted_index_unigram
        for term,value in self.inverted_index_unigram.items():
            if term in self.query:
                print 'TERM: ' + term
                print 'Document ID-->  TF   IDF   (TF * IDF)'
                idf = round((math.log((float(self.total_doc) / len(self.inverted_index_unigram[term])), 2)), 2)
                # for doc,tf in value.items():
                for doc in sorted(self.inverted_index_unigram[term], key=self.inverted_index_unigram[term].get,
                                  reverse=True):
                    # print doc
                    if doc in self.docs:
                        tf = self.inverted_index_unigram[term][doc]

                        prod = round(tf * idf, 2)
                        print doc + ' --> ' + str(tf) + '   ' + str(idf) + '    ' + str(prod)
